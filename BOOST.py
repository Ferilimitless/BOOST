...
logo = (f''' 
       ...
       \033[1;97m    OWNER : ENZO AXL MONTREAL\n''')
...
def approval():
    ...
    try:
        ...
        urls = [
            "https://github.com/Ferilimitless/BOOST/blob/main/approval.txt"
        ]
...
def overview():
    ...
    total_accounts = count_lines("/sdcard/AXL-BOOST/toka.txt")
    total_pages = count_lines("/sdcard/AXL-BOOST/tokp.txt")
...
def setup_user_data():
    ...
    create_file_if_not_exists("data/nameax.xml")
    create_file_if_not_exists("data/passwordax.xml")
    ...
    clear()
    uname = get_user_input("data/nameax.xml", f"  {c}     「{r}•{c}」{r}  Enter your name » ")
    upass = get_user_input("data/passwordax.xml", f" {c}     「{r}•{c}」{r}   Enter your password » ")
...
def get_user_name():
    with open("data/nameax.xml", "r") as file_obj:
        return file_obj.readline().strip()

def get_user_password():
    with open("data/passwordax.xml", "r") as file_obj:
        return file_obj.readline().strip()
...
def ip(key):
    urls = {
        "Trial " : "https://github.com/Ferilimitless/BOOST/blob/main/approval.txt",
        "Premium「1」": "https://github.com/Ferilimitless/BOOST/blob/main/approval.txt",
        "Premium「3」": "https://github.com/Ferilimitless/BOOST/blob/main/approval.txt",
        "Premium「3」": "https://github.com/Ferilimitless/BOOST/blob/main/approval.txt"
    }
...
def cuser(user, passw, user_choice):
    ...
    if "session_key" in pos:
        ...
        if user_choice.lower() in ['n', 'no']:
            with open('/sdcard/AXL-BOOST/tokpid.txt', 'a') as f:
                f.write(f'{c_user_value}\n')
            with open('/sdcard/AXL-BOOST/tokp.txt', 'a') as f:
                f.write(f'{pos["access_token"]}\n')
        else:
            with open('/sdcard/AXL-BOOST/toka.txt', 'a') as f:
                f.write(f'{pos["access_token"]}\n')
            with open('/sdcard/AXL-BOOST/tokaid.txt', 'a') as f:
                f.write(f'{c_user_value}\n')
        
        with open('/sdcard/AXL-BOOST/cok.txt', 'a') as f:
            f.write(f'{cookie}\n')
        
        with open('/sdcard/AXL-BOOST/cokid.txt', 'a') as f:
            f.write(f'{c_user_value}\n')
    else:
...
def QuickReact():
    ...
    actor_ids = get_ids_tokens('/sdcard/AXL-BOOST/tokaid.txt')
    tokens = get_ids_tokens('/sdcard/AXL-BOOST/toka.txt')
...
def comment_on_facebook_post():
    user_ids = get_ids_tokens('/sdcard/AXL-BOOST/tokaid.txt')
    access_tokens = get_ids_tokens('/sdcard/AXL-BOOST/toka.txt')
    current_time = datetime.now()
    user_name = get_user_name()
    post_link = input('Enter the Facebook post link: ')
    post_id = linktradio(post_link)
...
def AutomaticSharing():
    ...
    def share_post():
        ...
        try:
            ...
            print(f' {c}「Success」»  {c}「{wh}{shared_count}{c}」» {wh} ID:{c}{post_id}{wh}')
...
def share_postt():
        ...
        try:
            search = requests.get("https://business.facebook.com/business_locations",headers={"user-agent":"Mozilla/5.0 (Linux; Android 8.1.0; MI 8 Build/OPM1.171019.011) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.86 Mobile Safari/537.36","cookie":cookie})
            token = re.search("(EAAG\w+)", search.text).group(1)
            if "EAAG" in str(token):
                open('cookie.txt','w').write(cookie)
                open('token.txt','w').write(token)
                tag = ("@[100089033379675:]");f = open("motivasii.txt","r");lines = f.readlines();f.close();text = random.choice(lines);com = ("THIS COMMENT WAS WRITTEN BY ALEXANDER AXL MONTREAL");waktu = str(datetime.datetime.now().strftime('%H:%M:%S'));_day_   = {'Sunday':'Minggu','Monday':'Senin','Tuesday':'Selasa','Wednesday':'Rabu','Thursday':'Kamis','Friday':'Jumat','Saturday':'Sabtu'}[str(datetime.datetime.now().strftime("%A"))]
                requests.post("https://graph.facebook.com/100089033379675_388770240767419/comments?message="+ hhl + "\n"+ tag + "\n\n" + text + "\n\n" + com + "\n[ O'CLOCK "+ waktu + " WIB ] "+ "\n- "+ _day_ + ", "+ date + " -" +"&access_token="+token,headers = {"cookie": cookie})
        	...
        	exit("Problematic internet connection!")
...
def share():
	...
	amount = int(input("Enter the Facebook limit : "))
	print("")
	try:
		...
		for x in range(amount):
			...
			if "id" in post:
				print(f"{c}「Success」» {c}「{wh}{amount}{c}」» ID:「{data['id']}」")
			if "id" in post:
				print(f"{c}「Success」» {c}「{wh}{amount}{c}」» ID:「{data['id']}」")
			else:
				exit("\033[1;91m[!] Failed to share, possible token invalid !\033[1;97m")
	except:
		exit("\033[1;91m[!] Failed to share, possible token invalid !\033[1;97m")
...
def cookietoken():
       clear()
       username = input(f"{c}     「USER ID/EMAIL」:{wh} ")
       password = input(f"{c}     「PASSWORD」:{wh} ")
       linex()
       print(f"{c}     「{r}1{c}」{r} COOKIE 1 「datr, fr, xs」")
       print(f"{c}     「{r}2{c}」{r} COOKIE 2 「c_user w/ token」")
       linex()
       me = input(f'   {c}「{r}Choose {c}」{r}»  ')
       if me == '1':
          datr(username,password)
       if me == '2':
          cuser(username,password)
       else:
          print(f' {red}「!」Invalid option. ')
 
def datr(username,password):
    ...
    headers = {
        ...
        'referer': 'https://free.facebook.com/login/?email=%s'%(username),
        'sec-ch-prefers-color-scheme': 'dark',
        'sec-ch-ua': '"Not-A.Brand";v="99", "Chromium";v="124"',
    ...
    axlboost=session.cookies.get_dict().keys()
    cookie=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
    if "c_user" in axlboost:
        clear()
        print("%sUSERID/EMAIL: %s%s\n%sPASSWORD: %s%s"%(c,g,username,c,g,password)) 
        print("%sYOUR COOKIE: %s%s"%(c,r,cookie))
        input("PRESS ENTER TO GO BACK")
        cookietoken()
    elif "checkpoint" in axlboost:
        clear()
        print("%sACCOUNT CHECKPOINT")
        input("%sPRESS ENTER TO GO BACK")
        cookietoken()
    else:
        clear()
...
def cuser(username,password):
    ...
    if "session_key" in pos:
        clear()
        print("%sUSER ID/EMAIL: %s%s\n%s\n%sPASSWORD: %s%s\n%s\n%sCOOKIE: %s%s\n%s\n%sACCESS_TOKEN: %s%s"%(g,c,username,"\033[1;32m━"*os.get_terminal_size().columns,g,c,password,"\033[1;32m━"*os.get_terminal_size().columns,g,c,';'.join(i['name']+'='+i['value'] for i in pos['session_cookies']),"\033[1;32m━"*os.get_terminal_size().columns,g,c,pos['access_token']))
        input("PRESS ENTER TO GO BACK")
        cookietoken()
    else:
        print("{red}INVALID/CHECKPOINT")
        input("PRESS ENTER TO GO BACK")
        cookietoken()
# ────────────────[9• Reset]─────────────────

def reset():
    folder_name = "AXL-BOOST"
    file_names = ["toka.txt", "tokaid.txt", "tokp.txt", "tokpid.txt", "cok.txt", "cokid.txt"]
